...
def extract_information(result,result_1):
    ...
    if len(phone_numbers) >= 2:
        extracted_data['Primary Number'] = phone_numbers[0]
        extracted_data['Secondary Number'] = phone_numbers[1]
...
def insert_data(extracted_data):
    try:
        ...
        print("Error:", error)
def update_data(selected_row_name, new_values):
    try:
        # Execute an SQL UPDATE statement
        update_query = """
        UPDATE business_card
        SET name = %s, company_name = %s, primary_number = %s, secondary_number = %s,
